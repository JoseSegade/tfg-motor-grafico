...
interface Jugada {
  jugada: string;
  ficha: FuncionalidadArrastrarPieza;
}

export default class FuncionalidadArrastrarPieza extends FuncionalidadArrastrarConClick {
  public posicionInicial: Vector3 = Vector3.zero;
  public casillaActual: Casilla;
  public funcionalidadTablero: FuncionalidadTablero;
  ...
  public puedeMoverseHacia(pos: Vector2): boolean {
    let dragMove: string;
    const casillaInicial = this.funcionalidadTablero.humanAsWhite ? Casilla.obtenerCasillaInversa(this.posicionInicial.xy) : Casilla.obtenerCasilla(this.posicionInicial.xy);
    const casillaFinal = this.funcionalidadTablero.humanAsWhite ? Casilla.obtenerCasillaInversa(pos) : Casilla.obtenerCasilla(pos);
    if (
      casillaFinal.y === 0 &&
      casillaInicial.y == 1 &&
      'P' === this.funcionalidadTablero.chessBoard[casillaInicial.y][casillaFinal.x]
    ) {
      //pawn promotion
      dragMove =
        '' +
        casillaInicial.x +
        casillaFinal.y +
        this.funcionalidadTablero.chessBoard[casillaFinal.y][casillaFinal.x] +
        'QP';
    } else {
      //regular move
      dragMove =
    ...
    const userPosibilities: string = this.funcionalidadTablero.posibleMoves();
    if (userPosibilities.replace(dragMove, '').length < userPosibilities.length) {
      this.funcionalidadTablero.makeMove(dragMove);
      Mensaje.enviar(this.MOVER_JUGADOR, this, {jugada: dragMove.trim(), ficha: this});
      this.funcionalidadTablero.flipBoard();
      this.funcionalidadTablero.printTablero();
      const jugada: string = this.funcionalidadTablero.alphaBeta(
        FuncionalidadTablero.globalDepth,
        1000000,
        -1000000,
        '',
        0,
      ...
      console.log(jugada);
      this.funcionalidadTablero.makeMove(jugada);
      this.funcionalidadTablero.flipBoard();
      this.funcionalidadTablero.printTablero();
