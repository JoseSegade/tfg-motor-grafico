...
export default class FuncionalidadTablero extends FuncionalidadBase {
    ...
    public main(): void {
        ...
        while ("A" !== this.chessBoard[this.kingPositionC/8][this.kingPositionC%8]) {this.kingPositionC++;}//get King's location
        while ("a" !== this.chessBoard[this.kingPositionL/8][this.kingPositionL%8]) {this.kingPositionL++;}//get king's location
        /*
         * PIECE=WHITE/black
         * pawn=P/p
         * kinght (horse)=K/k
    ...
    public alphaBeta(depth: number, beta: number, alpha: number, move: string, player: number) {
        //return in the form of 1234b##########
        const list: string = this.posibleMoves();
    ...
    public posibleMoves(): string {
        ...
        for (let i: number = 0; i < 64; i++) {
            switch (this.chessBoard[i/8][i%8]) {
                case "P": list += this.posibleP(i);
                    break;
                case "R": list += this.posibleR(i);
                    break;
                case "K": list+= this.posibleK(i);
                    break;
                case "B": list+= this.posibleB(i);
                    break;
                case "Q": list+= this.posibleQ(i);
                    break;
                case "A": list+= this.posibleA(i);
                    break;
            }
        }
    ...
    public posibleP(i: number) {
        ...
        for (let j: number =-1; j<=1; j+=2) {
            try {//capture
                if (this.chessBoard[r-1][c+j].charAt(0).toLowerCase() === this.chessBoard[r-1][c+j].charAt(0) && i>=16) {
    ...
    public sortMoves(list: string): string {
